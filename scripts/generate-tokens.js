 ...
 * Output: src/css/generated/tokens.css
 */
...
const OUTPUT_PATH = join(import.meta.dir, '../src/css/generated/tokens.css');
...
function tokenValueToCSS(token, tokens) {
  ...
  // If it's an array (cubicBezier), convert to CSS function
...
for (const [name, token] of tokenEntries(tokens.primitives.motion.duration)) {
  css += `  --motion-duration-${name}: ${token.$value};\n`;
}
...
for (const [name, token] of tokenEntries(tokens.primitives.focusRing)) {
  ...
  // shadow is a composite token with a literal CSS value — emit as-is
